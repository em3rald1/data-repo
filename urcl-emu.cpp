...
public:
    ...
    void loadHdd(vector<u16> d, int size)
    {
        for (int i = 0; i < size; i++)
        {
            hdd[i] = d[i];
...
void writeFileBytes(const char *filename, std::vector<unsigned char> fileBytes)
{
    ...
    std::copy(begin(fileBytes), end(fileBytes), std::ostreambuf_iterator<char>(file));
}

void write(const char *fname, u16 *data)
{
    std::ofstream file;
...
int main(int argc, char **argv)
{
    ...
    if (argv[1][0] == '-' && argv[1][1] == 'r')
    {
        EMU urcl;
        //cout << "ok1\n";
        std::ifstream d(argv[2], std::ios::binary);
        // cout << "ok2\n";
